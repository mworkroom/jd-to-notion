...
- 상태: Phase SOP-1~4 및 SOP-6 로컬 구현 완료 · Phase SOP-5와 실제 JANDI 다운로드 검증 미완료
...
JANDI 첨부파일 다운로드와 파일명 정규화는 Phase SOP-6에서 구현했다. 파일명에서 사람 이름을 새로 추측하지 않고, 같은 JANDI 본문을 기존 파서로 분석해 얻은 `studentName`을 해당 요청의 기준 학생명으로 사용한다.

다운로드가 완료된 SOP Word 파일은 다음 계약에 따라 `학생명_정리된 원래파일명.docx` 형식으로 변경한다.
...
기존 JANDI 가져오기 흐름이 게시글에서 `.docx` 첨부파일 이름을 함께 전달한다. 브라우저 분석이 SOP 요청을 확정하면 로컬 서버가 해당 이름과 일치하는 다운로드만 2분간 기다리고, 임시 다운로드가 끝나 최종 파일 크기가 안정된 뒤 이름을 변경한다. 다운로드 폴더의 다른 Word 파일은 변경하지 않는다. 기본 감시 경로는 Windows `Downloads`이며, 다른 경로는 `.env`의 `JANDI_DOWNLOAD_DIR`로 지정한다.
...
- 같은 JANDI 요청에서 선택한 SOP Word 첨부파일만 처리
...
로컬 구현과 자동 테스트, 브라우저의 다운로드 대기 상태 검증은 완료했다. JANDI가 원격 디버깅 포트로 실행된 실제 환경에서 첨부파일 이름 전달과 다운로드 후 변경까지 확인하는 작업은 남아 있다.
